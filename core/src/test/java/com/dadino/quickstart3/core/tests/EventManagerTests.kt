...
class EventManagerTests {
  ...
  fun addEventSource_eventSourceAdded() {
	...
	eventManager.attachEventSource("empty", Observable.empty())
  ...
  fun addEventSources_eventSourcesAdded() {
	...
	eventManager.attachEventSources(mapOf("empty1" to Observable.empty(), "empty2" to Observable.empty()))

	assertEquals(2, eventManager.getEventSources().size())
  }

  @Test
  fun addEventSources_eventSourcesAddedTwice_noDuplicates() {
	assertEquals(0, eventManager.getEventSources().size())

	eventManager.attachEventSources(mapOf("empty1" to Observable.empty(), "empty1" to Observable.empty()))
