...
sealed class FlowAdvancement<STATE>(val advancementType: AdvancementType) {
  ...
  class GoToStep<STATE, STEP : FlowStep<STATE>>(val steps: List<STEP>) :
	  FlowAdvancement<STATE>(AdvancementType.Enter) {
	constructor(step: STEP) : this(listOf(step))
  }

  class GoBackToStep<STATE, STEP : FlowStep<STATE>>(val steps: List<STEP>) :
	  FlowAdvancement<STATE>(AdvancementType.Exit) {
	constructor(step: STEP) : this(listOf(step))
  }
...
class CloseFlow(val results: Map<String, Any> = mapOf()) : Signal()