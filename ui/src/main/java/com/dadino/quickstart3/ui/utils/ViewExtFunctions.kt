...
import com.dadino.quickstart3.icon.drawContextDrawable
...
fun ImageView.setIcon(icon: ContextDrawable?) {
  drawContextDrawable(icon)
}

fun MaterialButton.setIcon(icon: ContextDrawable?) {
  if (icon == null) {
	this.icon = null
  } else {
	this.icon = icon.getDrawable(this.context)?.mutate()
  }
}
